...
cursor.execute("WITH medals_count AS (SELECT medals.country, SUM(medals.gold + medals.silver + medal.bronze) AS total_medals FROM medals GROUP BY country), total_participants_count AS (SELECT athlete.country, COUNT(DISTINCT athlete_id) as total_participants FROM olympic_game_participants JOIN athlete ON olympic_game_participants.athlete_id = athlete.athlete_id GROUP BY country) SELECT medals_count.country, (medals_count.total_medals / total_participants_count.total_participants) AS medals_participants_ratio FROM medals_count JOIN total_participants_count ON medals_count.country = total_participants_count.country ORDER BY medals_participants_ratio DESC LIMIT 10")
print(cursor.fetchmany(size=10))